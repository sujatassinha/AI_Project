...
# total unique characters in input file
n_vocab = len(chars)    
...
model = Sequential()
model.add(LSTM(512, input_shape = (max_len, len(chars)))) #LSTM layer with 128 units        
...
def myGenerate(length_given, diversity_given):
    ...
    sent = []           
    input_taken = input('Enter first line of poem (min 3 chars):  ')
    while(len(input_taken) < 0): # since the sentence length is predefined,
...
print(myGenerate(500, 3.0)) # Generates text with given #of characters
                             # and text
